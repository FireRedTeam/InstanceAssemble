# modified from diffusers/models/transformers/transformer_flux.py
...
class LayoutFluxSingleTransformerBlock(nn.Module):
    def __init__(self, dim: int, num_attention_heads: int, attention_head_dim: int, mlp_ratio: float = 4.0, attention_type="default"):
        super().__init__()
    ...
    def forward(
        ...
        layout_hidden_states=None,layout_masks=None,img_idxs_list_list=None,layout_rotary_emb=None,layout_scale=1.0
    ) -> torch.Tensor:
        ...
        if self.attention_type == "layout" and layout_scale!=0.0:
            ...
            img_add_cnt = torch.zeros((bsz, img_len)).to(hidden_states_add)
            ...
            if valid_indices.size(0) > 0:
                ...
                for k in range(valid_indices.size(0)):
                    ...
                    hidden_states_layout = torch.cat([layout_hidden_states[i][j].unsqueeze(0).unsqueeze(0), hidden_states[i, img_idxs].unsqueeze(0)], dim=1)

                    with enable_lora((self.norm.linear, self.proj_mlp)):
                        norm_hidden_states, gate = self.norm(hidden_states_layout, emb=temb[i:i+1])
                        mlp_hidden_states = self.act_mlp(self.proj_mlp(norm_hidden_states))
                    with enable_lora((self.attn.to_q, self.attn.to_k, self.attn.to_v)):
                        attn_output = self.attn(
                            hidden_states=norm_hidden_states,
                            image_rotary_emb=layout_rotary_emb[k],
                        )

                    hidden_states_layout = torch.cat([attn_output, mlp_hidden_states], dim=2)
                    gate = gate.unsqueeze(1)
                    with enable_lora((self.proj_out,)):
                        hidden_states_layout = gate * self.proj_out(hidden_states_layout)

                    layout_output, img_output = (
                        hidden_states_layout[:, : 1],
                        hidden_states_layout[:, 1 :],
                    )
                    scaled_output = layout_scale * self.layout_forward(img_output.squeeze(0))  # [num_img_idxs, ...]
                    all_scaled_outputs.append(scaled_output)
                    all_layout_outputs.append(layout_output.squeeze(0,1))
            ...
            hidden_states = residual + hidden_states_add / img_add_cnt.unsqueeze(-1)
...
@maybe_allow_in_graph
class LayoutFluxTransformerBlock(nn.Module):
    def __init__(
        self, dim: int, num_attention_heads: int, attention_head_dim: int, qk_norm: str = "rms_norm", eps: float = 1e-6, attention_type="default"
    ):
        super().__init__()
...
class LayoutFluxTransformer2DModel(
    ...
    def __init__(
        ...
        self.transformer_blocks = nn.ModuleList(
            [
                LayoutFluxTransformerBlock(
                    dim=self.inner_dim,
                    num_attention_heads=num_attention_heads,
                    attention_head_dim=attention_head_dim,
                    attention_type=attention_type if (i % 3 == 0) else "default",
                )
        ...
        self.single_transformer_blocks = nn.ModuleList(
            [
                LayoutFluxSingleTransformerBlock(
                    dim=self.inner_dim,
                    num_attention_heads=num_attention_heads,
                    attention_head_dim=attention_head_dim,
                    attention_type=attention_type if (i == 0) else "default",
                )
    ...
    def forward(
        ...
        layout_raw_list, layout_text_embeddings_list, layout_text_ids_list, layout_masks_list, img_idxs_list_list = [], [], [], [], []
        for layout in layout_kwargs:
            ...
            layout_text_ids_list.append(layout_args["text_ids"])
            layout_masks_list.append(layout_args["masks"])
            img_idxs_list_list.append(layout_args["img_idxs_list"])
        layout_raw = torch.cat(layout_raw_list, dim=0)
        layout_text_embeddings = torch.cat(layout_text_embeddings_list, dim=0)
        layout_text_ids = torch.cat(layout_text_ids_list, dim=0)
        ...
        layout_rotary_emb_list = []
        img_len = img_ids.shape[0]
        valid_mask = (layout_masks == 1)
        valid_indices = valid_mask.nonzero(as_tuple=False)
        if valid_indices.size(0) > 0:
            valid_bs = valid_indices[:, 0]
            ...
            for k in range(valid_indices.size(0)):
                i = valid_bs[k].item()
                j = valid_objs[k].item()
                img_idxs = img_idxs_list_list[i][j]
                ids = torch.cat((layout_text_ids[i][j].unsqueeze(0), img_ids[img_idxs]), dim=0)
                layout_rotary_emb_list.append(self.pos_embed(ids))
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            if torch.is_grad_enabled() and self.gradient_checkpointing:
                encoder_hidden_states, hidden_states, layout_hidden_states = self._gradient_checkpointing_func(
                    block,
                    hidden_states,
                    encoder_hidden_states,
                    temb,
                ...
                encoder_hidden_states, hidden_states, layout_hidden_states = block(
                    ...
                    layout_rotary_emb=layout_rotary_emb_list,
